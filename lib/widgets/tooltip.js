...
export var actions = {
	...
	tooltip({ options, editor, next, timer }) {
		...
		const { text } = options;
		const pos = resolvePosition(options.pos, editor);
		const callback = () => {
      timer(() => {
        hide(() => timer(next));
      }, options.wait);
	 ...
	 */
	showTooltip({ options, editor, next }) {
		options = extend({
			pos: 'caret'  // position where tooltip should point to
		}, wrap('text', options));
	...
	showControlledTooltip({ options, editor, back, next, isFirstAction, isLastAction }) {
    options = extend({
      pos: 'caret'  // position where tooltip should point to
    }, wrap('text', options));
    ...
    const pos = resolvePosition(options.pos, editor);
		const callback = () => {
      if (!isFirstAction) {
      	const backButton = document.getElementById('js-tooltip-controls-back');
        backButton.removeAttribute('disabled');
        backButton.addEventListener('click', back);
      }

			if (!isLastAction) {
        const nextButton = document.getElementById('js-tooltip-controls-next');
        nextButton.removeAttribute('disabled');
        nextButton.addEventListener('click', next);
      }

      if (isLastAction) {
        const finishButton = document.getElementById('js-tooltip-controls-finish');
...
export function show({ text, pos, callback, hasBackControl = false, hasNextControl = false, hasFinishButton = false }) {
	hide();
